            ...
            <x-table wire:loading.class="opacity-75">
                <x-slot name="header">
                    ...
                    <x-table.header sortable wire:click.prevent="sortBy('number')"
                        :direction="$sortField === 'number' ? $sortDirection : null">{{ __('Booking ID') }}</x-table.header>
                    <x-table.header sortable wire:click.prevent="sortBy('name')"
                        :direction="$sortField === 'name' ? $sortDirection : null">{{ __('Status') }}</x-table.header>
                    <x-table.header sortable wire:click.prevent="sortBy('capacity')"
                        :direction="$sortField === 'capacity' ? $sortDirection : null">{{ __('Response Data') }}</x-table.header>
